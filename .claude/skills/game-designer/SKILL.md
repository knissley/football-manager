...
- **Unknowns per milestone**, for the milestone in flight and the one ahead: the questions
  whose answers change what gets built. Each goes into the body, not this comment: a
  question you can state precisely joins the queue, marked for who answers it (duty 3);
  one you cannot yet phrase that sharply goes under *Not yet specified*. The review names
  what graduated from fog to queue this session.
...
promote it), or **rejected** (a line in the *Ruled out* table of the same file, with the
reason and what would have to be redrawn for it to return, so it is not re-pitched).
...
While the orchestrator runs milestone N, you deepen N+1.

**Chart before you deepen.** If the milestone ahead has more unknowns than decisions
(count them: its roadmap section and the decisions that name it, against the questions
you can list), its first act is a charting session with the owner: `/design-grill` in
*chart* mode, breadth-first, fanning out across everything the milestone must decide
rather than going deep on any thread. Its output is a fog list and the first sharp
questions, and it goes into the tracker's Current state. A milestone whose systems already
have design docs, as M2's do, does not need one; say which it is and why.

**Every question is marked for who answers it.** *AFK* if the reference, the tree, or a
sourced season can answer it: you answer those yourself, by reading, and record the answer
in the review and wherever it belongs (a decision line, a brief section, a reference entry
to be filed). *HITL* if the owner's answer is the design: those alone go to the grill
queue, each marked *grill* (the ladder) or *prototype* (a cheap concrete artifact to react
to, when the question is how it should look or behave and words are stalling). Only a
HITL question costs the owner a session.

For each milestone the roadmap carries three things you keep current:
...
3. **Every idea has a home.** Milestone, size, dependency, stream requirement. No home,
   no acceptance. The parking lot is a home, and so is its *Ruled out* table.
...
rewritten in place and carries, in this order: `main` sha and date, and the implementation
tracker it reads; the milestone ahead, and whether it is charted, needs charting, or does
not need it; the grill queue, in order, each entry marked *grill* or *prototype*; the AFK
questions you owe answers to; *Not yet specified*, per milestone (the fog: in scope, not
yet sharp enough to queue; the test is whether you can state the question precisely, not
whether you can answer it); briefs awaiting a verdict; drift owed to the roadmap and
decisions; the stream list's status per milestone; questions waiting on the owner with
your recommendation. Nothing that describes where things stand goes in a comment.
...
**Fog to queue:** <questions that became sharp this session, each marked grill, prototype or AFK>.
**AFK answered:** <question — answer — where it is recorded>.
