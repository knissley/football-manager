...
struct DefensiveCallTests {
    ...
    @Test("Deep help is counted, not assumed", .tags(.unit))
    func deepDefenders() {
    ...
    @Test("Man and zone are distinguished, because rubs only beat one of them", .tags(.unit))
    func manCoverages() {
    ...
    @Test("Pressure coverages leave no deep help", .tags(.unit))
    ...
    @Test("Rusher counts match the names", .tags(.unit))
    func rusherCounts() {
    ...
    @Test("Only extra rushers count as a blitz", .tags(.unit))
    ...
    @Test("All-out means more rushers than deep help", .tags(.unit))
    func allOut() {
    ...
    @Test("Calls that concede short yardage say so", .tags(.unit))
    ...
    @Test("A two-minute sound call keeps help deep and does not vacate the middle", .tags(.unit))
    ...
    @Test("The named calls offer genuinely different trades", .tags(.unit))
    ...
    @Test("Vulnerabilities read the way the sport does", .tags(.unit))
    ...
    @Test("Every vulnerability is reachable", .tags(.contract))
    ...
    @Test("Named calls describe the defence their name claims", .tags(.unit))
    ...
    /// not one of these.
    @Test("Sub packages carry enough defensive backs for their coverage", .tags(.unit))
    ...
    /// pick between them.
    @Test("Situational reads separate the calls a coordinator would choose between", .tags(.unit))
