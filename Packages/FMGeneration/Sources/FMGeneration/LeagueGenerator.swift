...
public enum LeagueGenerator {
    ...
    public static func league(
        shape: LeagueShape = .standard, using random: inout SplittableRandom
        ...
        var pools: [Region: [GeneratedCity]] = [:]
        // Sorted so generation order — and therefore the whole world — is stable.
        for region in regions.sorted(by: { $0.rawValue < $1.rawValue }) {
            guard let count = demand[region], count > 0 else { continue }
            pools[region] = TeamGenerator.cities(
                in: region, count: count, ledger: &ledger, using: &random)
        }
        ...
        for conferenceIndex in 0..<shape.conferences {
            ...
            for divisionIndex in 0..<shape.divisionsPerConference {
                ...
                for _ in 0..<shape.teamsPerDivision {
                    guard let city = pools[region]?.popLast() else { continue }
                    let team = TeamGenerator.team(
                        id: identifiers.allocate(), city: city, ledger: &ledger, using: &random)
                    teams.append(team)
