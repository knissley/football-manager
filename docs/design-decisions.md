...
| 170 | **A new world tops out at heated** | The seeded past gives texture; the first genuine blood feud should be one the player caused. `bitter` is reachable through lived history, and tested to be, so the band is not dead. **Enforced by [#64](https://github.com/knissley/football-manager/issues/64):** nothing capped the draw, so nine of the first sixty worlds opened with a bitter rivalry in them. `RivalryGenerator.history(for:)` now drops the event doing the most work now — one at a time, never the event that earned an earned origin — until the pair opens below the band. In the generator and not in `heat(in:)`, because clamping the projection would clamp lived history with it and kill the band |
