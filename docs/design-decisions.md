...
| 224 | **Substitution is two mechanisms, not one: attrition rotates, situation assigns** | [Decision 177](#the-crude-engine) treated every non-starter-only position the same way — a per-snap draw against `RotationProfile`'s curve. Two different football facts were wearing one mechanism. **Attrition** positions (edge, defensive tackle, back, and the depth behind receiver, tight end, linebacker and the secondary) rotate because men tire and the sport spells them, so who plays is genuinely drawn and the curve is the right shape for it. **Situational** positions are a job assignment: the back on a pass concept and the receiver who lines up inside are chosen because of what the down asks for, not because a draw came up, so they are assigned deterministically from the depth chart and not drawn at all. The split is what makes a role mean something — a role the player assigns and a share the engine draws are different promises, and collapsing them is how "who plays" and "who is best" became the same fact. **The constraint this puts on [M3.5](roadmap.md#m35--roles-and-a-lineup-you-can-move-players-around-in):** rekeying `DepthChart` and `RotationProfile` from position to role must keep the attrition draw for the attrition positions. Drop it and the third and fourth edge rusher go to **zero snaps** — every `SlotLayout` package fields exactly two edges, so with the shipped curve `[0.75, 0.62, 0.44, 0.19]` the realised shares are 68.9 / 61.6 / 47.4 / 22.1 (computed by exact enumeration over the draw in `Lineup.fill`, not measured), and deterministic assignment sends the last two to nothing. **No calibration row would notice.** All seven `snaps.*` bands are position-group aggregates — `row:snaps.frontSeven` is edge, interior and linebacker together — and two edges stand on the field on every defensive snap whichever two they are. The only guard today is `RotationProfileTests.rotationDepthVaries`, a `.unit` assertion on the *table*, in a suite the rekeying rewrites. **The curve's shape stays unsourced and is described as such**, which the September 2026 audit already called out: what is checked is its *sum* — roughly how many of a position stand on the field at once — not the individual shares. Settled by [the snap-share and role-assignment brief](design/briefs/snap-share-and-role-assignment.md), 2026-09-15, on [#183](https://github.com/knissley/football-manager/issues/183) |
...
| 219 | **A read order lives on the five pass families until designs exist** | Decided on [#169](https://github.com/knissley/football-manager/issues/169). Designed plays are M6, after M5, in the role vocabulary M3.5 defines; building them for a read order would be M6 out of order, an identifier allocator ADR-0010 just removed, and a caller that picks designs. The families deliver the mechanism and the sentence — *his first read was the No. 1 receiver, and he never came off him*, naming the player — on the same record contract designs arrive on ([ADR-0010](adr/0010-plays-designs-and-calls.md), second amendment). Rejected: designed plays now, and a table on `PlayConcept`, which stays route-free |
| 220 | **The table is the crude resolver's, in `FMSimulation`, and dies with it** | `ReadProgression` generalises the one hold and depth per family the resolver had into an ordered list of reads over the roles its personnel can name — first, second and third receiver, tight end, back — with a distinguished checkdown. Keyed by this engine's slots for the reason `SlotLayout` is; M5's reads come from a design's assignments and its timing from geometry. M6's premade library is where the five authored orders are reused |
| 221 | **Each read carries a break time and a depth, and the ball is out at the break of the read thrown to** | The passer judges a read at its break, because that is where separation is created and when the ball has to be out; the pocket verdict compares the first arrival with that moment, so pressure is per snap rather than per family. Two anchoring rules: the hold a family had is its last read's break, the depth it had is its first read's — so the ball never comes out later than it did and `row:pressureRate` can only fall, bounded by `row:firstReadShare`. Rejected: one hold with an order walked at it, which leaves the read points' ticks invented, the checkdown at the family's depth, and C3's "pressure before a read clears" with nothing to compare against. Left open: a passer who locks on holds the read rather than forcing it, so a throw into coverage is only ever a misjudged window; whether he should sometimes force it is a question for the catch model and the retune |
| 222 | **A read order is authored coaching design, stated as such, and graded by what it produces** | Not in the rulebook and not a rate, so rule 10 has nothing for it to cite; registered beside the caller's run shares and the arrival window under *what a test claims about a game and nothing sources*. The table's own tests are `.pin` and `.contract`; the football tests are C3's grounding scenario and, once E8 [#179](https://github.com/knissley/football-manager/issues/179) bands it, target share by position group. The order is fixed per family, the five attacking different first reads; a drawn primary was rejected as a new draw and a knob E3 would own |
| 223 | **The defence gets no structure against a read yet, and no read carries an area** | C1's arrival and the coverage contest already disrupt an order; scheming against a read — taking the first one away — is `CallVulnerability`'s job, which the roadmap defers to M5 because a soft spot cannot honestly be soft without geometry. An area tag now would invent route vocabulary M6 owns |
...
| 177 | **Every snap credits a real player, with real rotation** | Backs share carries, the defensive line rotates heavily, the offensive line barely at all. Snap counts look real and a backup breaking out is possible. `DepthChart` is FMCore machinery M4 and M5 need anyway, not resolver code. **Reopened by [#21](https://github.com/knissley/football-manager/issues/21) and [#27](https://github.com/knissley/football-manager/issues/27):** the stream cannot report a snap count at all, and `Lineup.fill` redraws every slot per snap, so across 40 games the quarterback changed 125 times between consecutive dropbacks with no injury. **Half restored by [#21](https://github.com/knissley/football-manager/issues/21):** the stream reports a snap count again, and the lineup is drawn by the game rather than by the resolver, immediately before the snap is resolved. **[#27](https://github.com/knissley/football-manager/issues/27) is closed:** rotation is now a property of the position — the quarterback, the five line spots and the three specialists are one man's job and are not drawn at all, everything else still splits its snaps against the curve. **Amended 2026-09-15** by [the snap-share and role-assignment brief](design/briefs/snap-share-and-role-assignment.md) on [#183](https://github.com/knissley/football-manager/issues/183): "everything else splits its snaps against the curve" holds for the positions the sport rotates by **attrition** and is replaced by deterministic role assignment for the **situational** ones — see [decision 224](#situational-football-and-defense), which is where that split is stated. The original text stands; this note records what the split does to it, and one warning it carries. **The third edge rusher is load-bearing on the draw this decision describes.** Every `SlotLayout` package fields exactly two edges, so against the shipped curve `[0.75, 0.62, 0.44, 0.19]` the realised shares are 68.9 / 61.6 / 47.4 / 22.1 — computed by exact enumeration over the draw in `Lineup.fill`, not measured, and exact rather than approximate because for edge the slot count does not vary by package. If [M3.5](roadmap.md#m35--roles-and-a-lineup-you-can-move-players-around-in)'s rekeying replaces that draw with role assignment for edge, the third and fourth edge go to zero and this decision's promise — "the defensive line rotates heavily", "a backup breaking out is possible" — is silently withdrawn. No `snaps.*` band can see it: all seven are position-group aggregates, and two edges are on the field either way |
