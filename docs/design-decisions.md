...
| 224 | **A decision may not read information its real-world decider would not have** | Decision 42's rule, generalised past the tendency model to every draw the engine makes, on either side of the ball: at the moment a decision is made, would the real decider know this? Decided on [#218](https://github.com/knissley/football-manager/issues/218), where the defensive package could have been drawn conditioned on the offensive concept the engine had already drawn — which reaches the sport's rate on carries (18.8 / 18.3% of eleven personnel's first-and-ten designed carries meet a four-back front, 2023-24) by telling the defence the play call. The engine instead draws over the population the defence can identify — first-and-ten snaps of either kind, 14.2 / 14.0% — and *reports* the carry subset at 13.8 / 14.6% rather than targeting it; the ADR's table prints the counts behind all three rates. The cost is real: a target measured on a post-hoc population can be unreachable, a *Done when* written against one is wrong rather than missed, and a row can stay `OFF` where conditioning would close it ([ADR-0014](adr/0014-decide-from-what-the-decider-can-see.md)). Rejected: conditioning the package on the concept; leaving the pairing computed from the receiver count. Left open: an offence that re-checks its concept against the box it sees, which moves the boundary rather than crossing it and which the engine does not model |
