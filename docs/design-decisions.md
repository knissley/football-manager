...
- **Whether a stadium's roof, surface and capacity band are part of the curated
  identity.** [Decision 215](#world-generation) fixes the thirty-two franchises but not
  how much of the building comes with them. The recommendation recorded on
  [#69](https://github.com/knissley/football-manager/issues/69) is that they are: a dome
  team is part of a franchise's identity in the way its nickname is, and fixing the roof
  and surface mix fixes the weather mix the harness calibrates against, which is most of
  why the league is coming off the draw at all. Climate still follows the city, and
  [decision 151](#franchises) holds either way — a stadium is simulation input whether it
  was curated or drawn.
