...
66. A try is one untimed scrimmage down, so time is not in during it, and the ten-second
    ...
    nothing to run on a try: a two-point pass grounded inside two minutes is unsuccessful
    and not replayed, its yardage goes nowhere, no seconds go, and the kickoff is put in
    play from its ordinary spot at the clock the touchdown left.
    `[2025 · 3-40, 3-36-3, 4-7-1 Item 1, 11-3-3 Item 3-b]` —
...
70. Half the distance to the goal is measured from the spot of enforcement, whichever spot
...
71. Every foul is enforced from one of the spots the book lists — the previous spot, the
...
72. A flag that comes down before the ball is snapped walks off from the succeeding spot
...
73. The basic spot for a foul during a run not followed by a change of possession is the
...
74. Under the three-and-one method a defensive foul during a run is enforced from the basic
...
75. Its exception: an offensive foul behind the line of scrimmage is enforced from the
...
76. Between the snap and the moment a forward pass from behind the line is over, a foul by
...
77. A personal foul by the defence before a completion is enforced from the dead-ball spot
    or the previous spot, whichever favours the offence. `[2025 · 8-6-1-d]` —
    `test:roughingOnACompletion`
78. Defensive pass interference is a first down at the spot of the foul.
    `[2025 · 8-5-Penalty, 8-6-1-b]` — `test:spotFouls`, `test:interferenceDownfield`
79. Defensive pass interference in the end zone is first and goal at the 1, or half the
...
80. Offensive pass interference is ten yards from the previous spot, and the down is
    replayed. `[2025 · 8-5-Penalty]` — `test:offensiveInterference`
81. A defensive interference flag is the reason the pass was not caught: the foul is
...
82. Contact that would otherwise be interference is permissible when the pass is clearly
    ...
    **modelling**: the offence's downfield block is not drawn as an act of its own, so the
    ...
    nobody could reach.
83. A shove or a push-off that buys a receiver room is interference by the offence, and the
...
84. Illegal contact is a foul only while the passer is still back there with it. Both
...
85. Unsportsmanlike conduct after the play is fifteen yards from the succeeding spot, and an
    automatic first down when it is the defence's. `[2025 · 12-3-1]` —
    `test:conductFoulAfterThePlay`
86. When a run with a foul in it is followed by a change of possession, a personal foul by
...
87. A defensive foul during a touchdown leaves the score standing; an offensive contact foul
...
88. The non-penalised team takes whichever of the two outcomes is better for it, and a
    double foul with no change of possession offsets and replays the down.
    `[2025 · 14-5-1]` — `test:flagsAreEnforced`
89. A personal or unsportsmanlike foul during a down in which the opponent kicks a field
    ...
    `test:runningIntoTheKickerOnAMadeFieldGoalIsDeclined`
90. The basic spot when a **run** is followed by a change of possession is the spot where
...
91. A pass grounded costs the down as well as ten yards from the previous spot — second and
    ten becomes third and twenty — and on fourth down the lost down is the series, the
    defence taking over where the walk-off leaves the ball. `[2025 · 8-2-1, 8-2-Penalty]` —
...
92. An onside kick the kicking team legally recovers is its ball, first and ten, where the
...
93. Only a trailing team may attempt an onside kick, it must declare it, and it may do so at
...
94. A kickoff is from the kicking team's 35 and a safety kick from its 20 — unless a
...
95. A kick that reaches the end zone without coming down in the landing zone first is a
...
96. A kick that goes out of bounds or comes down short of the landing zone hands the
...
97. A returned kick changes hands where the return ended, and a kick returned all the way is
...
98. A series is four scrimmage downs to reach the line to gain, which sits ten yards
    ...
    `test:lossMovesBack`
99. Every ball is on a legal spot, in a legal down and distance, in a period the rules
    define. `[2025 · 1-1-1, 3-8-2, 4-1-1]` — `test:spotsAreAlwaysLegal`,
    `test:situationsAreValid`, `test:streamIsOrdered`
100. **Contract**, not a rule: the package a defensive call names is the package the
...
101. A team scores about what a real team scores, and the game's points come mostly from
...
102. Games end tied about as rarely as they really do, reach overtime about as often, and
    play about as much of the overtime period. — `row:tiesPerGame`, `row:overtimeRate`,
    `row:overtimeLength`
103. A team throws for and runs for about what a real team does, at about the same yards a
...
104. Passes are completed, pressured, sacked and intercepted at about the real rates. —
    `row:completionPercentage`, `row:sackRate`, `row:interceptionRate`, `row:pressureRate`,
    `row:completionsZeroOrFewer`
105. A game holds about as many snaps as a real one, and a team runs about as many plays from
    scrimmage. — `row:playsPerGame`, `row:playsFromScrimmage`
106. Third down comes up at about the real distance and is converted at about the real rate,
...
107. The shape of a carry is right and not just its mean: about as many are stuffed, gain two
    or fewer, reach ten, and break twenty. — `row:carriesStuffed`, `row:carries2orFewer`,
    `row:carries10plus`, `row:carries20plus`
108. The shape of a dropback is right too — losses, no-gains, ten, twenty and forty-plus. —
    `row:dropbackLoss`, `row:dropbackNoGain`, `row:dropback10plus`, `row:dropback20plus`,
    `row:dropback40plus`
109. Drives end in a punt, a touchdown or on downs at about the real rates, and there are
...
110. The shape of a drive is right: three-and-outs, short drives, middling ones and long
    ones. — `row:drives3orFewer`, `row:drives4to7`, `row:drives8plus`, `row:threeAndOut`
111. A trip inside the 20 ends in a touchdown about as often as it really does. —
    `row:redZoneTouchdownRate`
112. Drives start about where they really start, and about as often inside their own half. —
     `row:averageStart.2025`, `row:averageStart.2024`, `row:ownHalfStarts.2025`,
     `row:ownHalfStarts.2024`, `row:snapsInsideOwn10`
113. Teams punt about as often, for about the real gross and net, about as many punts come
...
114. Field goals are attempted about as often, from about the real spread of distances, and
...
115. Extra points are made, and two-point tries taken and converted, at about the real
     rates. — `row:extraPointsMade`, `row:twoPointTries`, `row:twoPointConversion`
116. Fourth down is punted, kicked and gone for at about the real rates, and converted at
...
117. The ball is turned over, lost and fallen on at about the real rates, and about as many
...
118. Kickoffs are returned and taken for touchbacks at the rates the kickoff rule in force
...
119. About as many flags fly as really do, spread over the fouls that really get called, and
...
120. Personnel looks like the sport's: 11 personnel against nickel most of the time, the
...
121. The endgame is played: teams kneel, spike, scramble and spend timeouts about as often
     as they really do. — `row:kneelsPerGame`, `row:spikesPerGame`, `row:scramblesPerGame`,
     `row:timeoutsPerGame`
122. Every man on the field is in the record, and each position group takes about as many
...
123. Over a season, team win totals spread about as widely as they really do. —
     `row:winTotalSigma`, which cannot be measured before there is a schedule, at M3
124. The middle of the run distribution is a large part of it: the ordinary carry gains three
     ...
     so the two rows of entry 107 grade the 42.3 floor between them** over four hundred
...
125. A carry of twenty yards or more has a broken tackle in front of it. A long run is a man
...
126. A completion is a completion whether it gained a yard, none, or lost one. The record
...
127. Players miss games at about the rate they really do, and heavy rain takes points off a
...
128. Receivers drop about as many as they really do, defenders knock away about as many,
...
129. A defensive interference flag on a pass that was then completed is the engine
