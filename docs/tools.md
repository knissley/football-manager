...
before the sweep that clears them, and no new history joined the list while that sweep
waited. The sweep has run: **one line is left**, in `PlayRecord.swift`, which was being
edited by other work at the time and is cleared with the rest of that file. Read what
is carried in the file itself rather than from a count here; the lint prints both
numbers on every run. Three tab-separated fields:
...
implementation that agreed on every figure. The figures are a snapshot of a history that
only grows; re-take them with
`FM_RULEBOOK_TEXT=… ./scripts/lint-reference.sh --messages --range <first>..<last>`
rather than reading them here as current. They are the second arm's whole justification:
...
plain-text extraction of the book, September 2026 — a snapshot of a tree that has grown
since, kept because what it settles is the choice of `n` and not the counts:
...
**The block below is an example of the shape, taken at the wave 1 merge — not the live
table.** The live one is in [testing.md](testing.md#the-census-as-it-stands), between
its `test-census` markers, regenerated by `--markdown` and diffed by CI; run the script
for today's.
