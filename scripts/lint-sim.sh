...
foundation	^[[:space:]]*(@[A-Za-z_]+[[:space:]]+)?import[[:space:]]+([a-z]+[[:space:]]+)?Foundation([^A-Za-z0-9_]|$)	import Foundation — the FM* modules are framework-free (ADR-0004)
...
for file in "${goldens[@]}"; do
    ...
    while IFS= read -r hit; do
        [ -n "$hit" ] || continue
        report "$file:${hit%%:*}: Hasher in a golden test — its seed is randomised per process, so the golden cannot detect drift (ADR-0003)"
    done < <(printf '%s\n' "$code" | grep -nE -- '(^|[^A-Za-z0-9_])Hasher([^A-Za-z0-9_]|$)' || true)
done
